...
class RelevanceCheck(BaseModel):
    ...
    summary: str = Field(description="Summary of domain concepts found, or empty string.")

class BoundedContextList(BaseModel):
    contexts: List[str] = Field(description="List of Bounded Context names identified in the document.")
...
class Entity(BaseModel):
    ...
    synonyms_to_avoid: Optional[List[str]] = Field(description="List of terms forbidden for this entity (e.g., Client, User).")

class ValueObject(BaseModel):
    name: str = Field(description="Name of the value object")
    attributes: List[str] = Field(description="List of attributes defining this value object")
...
class UbiquitousLanguage(BaseModel):
    ...
    value_objects: Optional[List[ValueObject]] = Field(description="Value objects in this context")
...
class BoundedContext(BaseModel):
    context_name: str = Field(description="Name of the bounded context (e.g., SalesContext).")
    description: str = Field(description="What this context is responsible for.")
    allowed_dependencies: Optional[List[str]] = Field(description="List of other contexts this context interacts with.")
...
class GlobalRules(BaseModel):
    naming_convention: Optional[str] = Field(default="PascalCase", description="Preferred naming convention (e.g., camelCase)")
    banned_global_terms: Optional[List[str]] = Field(default_factory=list, description="Terms banned across the entire project")
...
class DomainModel(BaseModel):
    ...
    project_metadata: ProjectMetadata = Field(description="Metadata about the generation")
    
    # Context listesi
    bounded_contexts: List[BoundedContext] = Field(description="List of all identified Bounded Contexts")
    
    # Global kurallar
    global_rules: Optional[GlobalRules] = Field(description="Project-wide architectural rules")