...
**Status summary (post-iteration-8):**
- 10 ingestion findings: 3 SHIPPED (F-3, F-5, F-2), 3 MAJOR-OPEN (F-1, F-4-uncertain), 3 MINOR-OPEN, 1 TRIVIAL-OPEN.
- 14 orchestrator findings: 5 SHIPPED (F-13, F-14, F-21, F-22, F-23), 1 MAJOR-OPEN-DORMANT (F-11), 7 MINOR-OPEN, 1 TRIVIAL-OPEN.
- **Total OPEN MAJOR (live): 2** (F-1, F-4-uncertain) + 1 DORMANT (F-11). Both layers reduced.
- **Iteration-9 recommendation:** F-1 (PDF defensive handling) — symmetric to WP-CORE-9 for `read_pdf`. Magic-byte detection pattern can be re-used; check `data.startswith(b"%PDF-")` to surface `MisLabeledFileError` when a `.pdf` extension wraps non-PDF content; also add typed exceptions for corrupted/password-protected/empty PDFs. Alternative: F-4 (TOC heuristic) — content-quality issue rather than crash path.

**Last refresh:** 2026-05-23 14:00 GMT+3 (post-WP-CORE-9)
