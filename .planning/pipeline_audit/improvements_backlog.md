...
| id | component | finding | severity | effort | blast | status |
|---|---|---|---|---|---|---|
| F-11 | core/architect.py | Parallel Scout (`scout_max_workers > 1`) rate-limit reentrancy: lock window in `_wait_for_rate_limit` (`:141-150`) allows microslip on consecutive workers — cumulative quota loss under N workers can be `(N-1) * (min_delay / N)`. Needs condition-variable or semaphore. | MAJOR | M-L | PIPELINE | OPEN |
...
| F-14 | core/architect.py + core/orchestration/pipeline.py | `SynthesizerEmptyModelError` escapes pipeline (pipeline.py:81-83 raises; no analogous degrade-handler unlike `RefinementExhaustedError` at lines 68-74). Hard-fail on a degenerate case — needs explicit policy. | MAJOR | M | PIPELINE | OPEN |
...
**Status summary (post-iteration-3):**
- 10 ingestion findings: 2 SHIPPED, 4 MAJOR-OPEN, 3 MINOR-OPEN, 1 TRIVIAL-OPEN.
- 11 orchestrator findings: 1 SHIPPED (F-13), 3 MAJOR-OPEN (F-11, F-14, F-21), 6 MINOR-OPEN (F-12, F-15, F-17, F-18, F-19, F-20-downgraded), 1 TRIVIAL-OPEN (F-16).
- **Total OPEN MAJOR: 7** (F-1, F-2, F-4-uncertain, F-11, F-14, F-21) — F-21 newly discovered during WP-CORE-4 spec drafting.
