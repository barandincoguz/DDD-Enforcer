...
**Status update (2026-05-22 21:15 GMT+3) — SHIPPED in WP-CORE-6 at `a86bbbb`.** Codex xhigh review reframed scope substantially (4 CRITICAL + 4 WARN):
- The original audit text named "Architect prompt + parsing change" but Codex C-1 (A7 final-model-loss) exposed a deeper issue: even with Architect populating IDs, Specialist rebuilt `ContextHypothesis(context_name=ctx_name, description="")` fresh at architect.py:707, discarding any IDs upstream. Final `DomainModel.bounded_contexts[].supporting_sentence_ids` would still have been empty post-v1.
- Fix shipped end-to-end across 3 production files: Architect prompt rewrite (numbered sentences + object array shape); strict-shape parser (rejects bare-string dict + top-level list per Codex W-2); line-pair-aware truncation `_truncate_numbered_pairs` (Codex W-1, preserves `[N]` atomicity under 50k char cap); `extract_per_context_details` signature widened from `List[str]` to `List[ContextHypothesis]` (Codex C-1 — preserves IDs into SpecialistAnalysis.context via re-using the input ctx instead of rebuilding); D1 also flags empty IDs as ungrounded_context ERROR (honest signal — full enforcement requires F-22 Refiner extension); degrade-log emits full exc.issues list (Codex C-4).
